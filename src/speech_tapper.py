...
def run():
    ...
    ax3.set_title("Commanded angles (deg)"); ax3.set_ylim(-30, 30); ax3.set_xlim(0, WINDOW_S)
    ...
    with ReachyMini() as mini:
        print("[robot] Ready. Continuous 6-DoF speech sway (no accents).")
        try:
            dt = 0.01  # 100 Hz
            while True:
                now = time.time()
                audio_time_now = an.frame_idx * (HOP_MS / 1000.0) if an.start_wall else 0.0
                ...
                if (now - last_draw) >= (1.0 / PLOT_HZ) and an.t_hist:
                    ...
                    if tx:
                        y0 = [-80]*len(tx); y1 = [0 if an.vad_on else -80]*len(tx)
