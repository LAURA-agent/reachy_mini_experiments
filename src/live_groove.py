...
class Config:
    ...
    unstable_periods_before_stop: int = 2
...
class SharedState:
    def __init__(self):
        ...
        self.smart_offset_enabled = False
    ...
    def toggle_smart_offset(self):
        self.smart_offset_enabled = not self.smart_offset_enabled

    def adjust_offset(self, amount):
        self.manual_offset += amount
    ...
    def adjust_amplitude(self, amount):
...
class Choreographer:
    ...
    def prev_move(self):
        self.move_idx = (self.move_idx - 1 + len(self.move_names)) % len(
            self.move_names
        )
        self.beat_counter_for_cycle = 0

    def next_waveform(self):
        self.waveform_idx = (self.waveform_idx + 1) % len(self.waveforms)
...
def keyboard_listener_thread(shared_state: SharedState, stop_event: threading.Event):
    def on_press(key):
        ...
        if hasattr(key, "char"):
            if key.char.lower() == "p":
                shared_state.toggle_smart_offset()
            if key.char.lower() == "n":
        ...
        if key == keyboard.Key.right:
            shared_state.adjust_offset(0.01)
...
def ui_thread(data_queue: Queue, config: Config, stop_event: threading.Event):
    ...
    while not stop_event.is_set():
        ...
        paused_status = (
            " | PAUSED (Music Unstable)" if last_data["unstable_pause"] else ""
        )
...
def main(config: Config) -> None:
    ...
    threading.Thread(
        target=audio_thread, args=(music, config, stop_event), daemon=True
    ).start()
    threading.Thread(
        target=keyboard_listener_thread, args=(shared_state, stop_event), daemon=True
    ).start()
    threading.Thread(
        target=ui_thread, args=(data_queue, config, stop_event), daemon=True
    ).start()
        ...
        try:
            while True:
                ...
                if changes["next_move"]:
                    choreographer.next_move()
                if changes["prev_move"]:
                    choreographer.prev_move()
                if changes["next_waveform"]:
                    choreographer.next_waveform()
                if changes["amplitude_change"]:
                    choreographer.change_amplitude(changes["amplitude_change"])
                ...
                active_bpm = (
                    librosa_bpm
                    if time.time() - last_event_time < config.silence_tmo
                    else 0.0
                )
                ...
                if new_beats and active_bpm > 0:
                    ...
                    while i < len(new_beats):
                        ...
                        if (
                            i + 1 < len(new_beats)
                            and (new_beats[i + 1] - current_beat) < min_interval
                        ):
                            c = new_beats[i + 1]
                ...
                if can_dance:
                    ...
                    if changes["smart_offset_toggled"]:
                        phase_error = calculate_phase_error(
                            t_beats, last_good_beat, active_bpm
                        )
                        correction = np.clip(
                            phase_error * config.offset_correction_rate,
                            -config.max_phase_correction_per_frame,
                            config.max_phase_correction_per_frame,
                        )
                        t_beats += beats_this_frame + correction
                    else:
                        phase_error = 0.0
                        t_beats = reference_t_beats
                ...
                ui_data = {
                    ...
                    "correction_on": changes["smart_offset_toggled"],
                    "manual_offset": changes["manual_offset"],
                    "unstable_pause": not is_stable_enough_to_continue
                    and state == "Unstable",
                }
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Reachy Rhythm Controller",
